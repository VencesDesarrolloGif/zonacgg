...
$response = array("status" => "success");
        ...
        $entidadTrabajoRS=getValueFromPost("entidadTrabajoRS");
	...
	try{
		$listaEmpleados= $negocio -> ListaReclutadoresByEntidad($lineaNegocio, $month,$puesto, $entidadTrabajoRS);
        $elementosReclutados="";
	       
        for ($i = 0; $i < count($listaEmpleados); $i++)
        {   
            $numeroEmpleado = $listaEmpleados[$i] ["numeroEmpleado"];
            $elementosReclutados= $negocio -> ElementosByReclutadorEntidad($numeroEmpleado, $month);
