...
<div >
  ...
  <button class="btn btn-primary" type="button" onclick="generartxt();"> Generar txt</button>
	...
	<!--  fin modal firma -->
  <div class="modal fade" tabindex="-1" role="dialog" name="modalConfirmacionCancelacionEnvioAlta" id="modalConfirmacionCancelacionEnvioAlta" data-backdrop="static" style="display: none;">
  	<div class="modal-dialog" role="document">
    	<div class="modal-content">
      	<div class="modal-header">
      		<div id="alertMsgDefinitivo" name="alertMsgDefinitivo"></div>
      	...
      	<div class="modal-body">
        	...
        	<label class="control-label label" for="selMotivoNoImss">Motivo</label>
            <select class="span3" id="selMotivoNoImss" name="selMotivoNoImss"></select>
            <br/>
      	</div>
...
<div class="modalEdit hide fade" tabindex="-1" role="dialog" name="modalFirmaAltaImss" id="modalFirmaAltaImss" data-backdrop="static" style="display: none">
  <div id="errormodalFirmaAltaImss"></div>
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
      ...
      <div class="modal-body" align="center">
        ...
        <input  type="text"  id="NumEmpModalFirmaAltaImss"  class="input-medium"  name="NumEmpModalFirmaAltaImss"  placeholder="00-0000-00 Ó 00-00000-00"/>
        <input  type="hidden"  id="NumEmpModalFirmaAltaImsshidden"  class="input-medium"  name="NumEmpModalFirmaAltaImsshidden"/>
        <span class="add-on">Contraseña</span>
        <input  type="password"  id="constraseniaFirmaAltaImss"  class="input-xlarge"  name="constraseniaFirmaAltaImss"  title="El campo identifica entre mayusculas y minusculas favor de considerarlo"/>
        <input  type="hidden"  id="constraseniaFirmaAltaImssHidden"  class="input-xlarge"  name="constraseniaFirmaAltaImssHidden"/>
        <input  type="hidden"  id="NumEmpAltaImssHidden"  class="input-xlarge"  name="NumEmpAltaImssHidden"/>
        <input  type="hidden"  id="txtNombrEmpleadoDefinitivoAltaImss"  name="txtNombrEmpleadoDefinitivoAltaImss"/>
        <input type="hidden" id="txtcaso" name="txtcaso" />
      </div>
      <div class="modal-body" align="center">
        <button  type="button"  id="btnFirmaAltaImss"  name="btnFirmaAltaImss"  onclick="RevisarFirmaAltaImss();"  style="display: block"  class="btn btn-primary">Firmar</button><br />
        <button type="button" id="btnCancelarFirmaAltaImss" name="btnCancelarFirmaAltaImss" onclick="cancelarFirmaAltaImss();" class="btn btn-danger">Cancelar</button>
      </div>
    </div><!-- /.modal-content -->
  </div><!-- /.modal-dialog -->
</div><!-- /.modal -->
...
$(document).on("click",".guardar",function(e)
{
	...
	function tabla(){
     		...
     		if (response.status == "success")
        {
          ...
          for ( var i = 0; i < empleadoEncontrado.length; i++ ){
            ...
            var FechaConfirmacion=empleadoEncontrado[i].confirmacion;
            var FechaRevision=empleadoEncontrado[i].revision;
            var idRechazado=empleadoEncontrado[i].idRechazado;
            var fechaAceptacion="";
            if(idRechazado == 2){
            	fechaAceptacion=FechaConfirmacion;
          	}else {
          		fechaAceptacion=FechaRevision;
          	}
       			if(tipoPeriodo == "1" || tipoPeriodo ==1){
            	var period = "QUINCENAL";
            }else{
            	var period = "SEMANAL";
            }
            ...
            $('#editinplace').append("<tr><td class='id'>"+numeroEmpleado+"</td><td>"+apellidoPaterno+"</td><td>"+apellidoMaterno+"</td><td>"+nombreEmpleado+"</td><td>"+idPinto+"</td><td>"+PuntoServ+"</td><td>"+Origen+"</td><td>"+fechaAceptacion+"</td><td>"+descripcionCategoria+"</td><td>"+period+"</td><td>"+numeroImss+"</td><td class='editable' data-campo='fechaImss'><span>"+fechaImss+"</span></td><td class='editable' data-campo='EntidadALaborar'><span>"+EntidadALaborar+"</span></td><td class='editable' data-campo='registroPatronal'><span>"+registroPatronal+"</span></td><td class='editable' data-campo='tipoTrabajador'><span>"+descripcionTipoTrabajadorImss+"</td><td class='editable' data-campo='salarioDiario'><span>"+salarioDiario+"</span></td><td>"+salarioBaseCotizacion+"</td></tr>");
          }                    
      ...
      success: function(response) {
        ...
        {         
          ...
          for ( var i = 0; i < empleadoEncontrado.length; i++ ){
            ...
            $('#editinplace').append("<tr><td class='id'>"+numeroEmpleado+"</td><td>"+apellidoPaterno+"</td><td>"+apellidoMaterno+"</td><td>"+nombreEmpleado+"</td><td>"+idPinto+"</td><td>"+PuntoServ+"</td><td>"+Origen+"</td><td>"+fechaAceptacion+"</td><td>"+descripcionCategoria+"</td><td>"+period+"</td><td>"+numeroImss+"</td><td class='editable' data-campo='fechaImss'><span>"+fechaImss+"</span></td><td class='editable' data-campo='EntidadALaborar'><span>"+EntidadALaborar+"</span></td><td class='editable' data-campo='registroPatronal'><span>"+registroPatronal+"</span></td><td class='editable' data-campo='tipoTrabajador'><span>"+descripcionTipoTrabajadorImss+"</td><td class='editable' data-campo='salarioDiario'><span>"+salarioDiario+"</span></td><td>"+salarioBaseCotizacion+"</td><td><img src='"+iconImss+"' class='cursorImg'  onclick='cambiarEstatusParaGenerarTxt(\"" + numeroEmpleado + "\","+emisionAltaImssConfirmada+");'></td><td><a  href='javascript:cancelarDefinitivamenteEnvioAltaImss(\"" + numeroEmpleado + "\",\"" + nombreCompletoEmpleado + "\");' class='cursorImg'>No emitir alta definitivamente</a></td></tr>");
          }                    
        }
        else if (response.status == "error" && response.message == "No autorizado"){
          //window.location = "login.php";
	...
	function cancelarDefinitivamenteEnvioAltaImss(numeroEmpleado, nombreCompletoEmpleado){
		$("#modalFirmaAltaImss").modal();
		$("#txtEmpleadoSinImssDefinitivo").val(nombreCompletoEmpleado);
		$("#txtNumEmpleadoDefinitivo").val(numeroEmpleado);
	}

	function RevisarFirmaAltaImss() {
  	var NumEmpModalBaja = $("#NumEmpModalFirmaAltaImss").val();
 		var constraseniaFirma = $("#constraseniaFirmaAltaImss").val();
  	if (NumEmpModalBaja == "") {
    	cargaerroresFirmaInternaImssAlta("El numero de empleado no puede estar vacio");
  	} else if (constraseniaFirma == "") {
    	cargaerroresFirmaInternaImssAlta("Escriba la contraseña para continuar");
  	} else {
    	$.ajax({
      	type: "POST",
      	...
      	success: function (response) {
        	if (response.status == "success") {
          	...
          	if (RespuestaLargo == "0") {
            	cargaerroresFirmaInternaImssAlta("La Contraseña ingresada es incorrecta favor de escribirla exactamente como la ingreso en el registro");
          	}else{
            	var contraseniaInsertadaCifrada = response.datos["0"].ContraseniaFirma;
            	$("#constraseniaFirmaAltaImssHidden").val(contraseniaInsertadaCifrada);
            	$("#NumEmpModalFirmaAltaImsshidden").val(NumEmpModalBaja);
            	$("#modalFirmaAltaImss").modal("hide");
            	$("#NumEmpModalFirmaAltaImss").val("");
            	$("#constraseniaFirmaAltaImss").val("");
            	$("#modalConfirmacionCancelacionEnvioAlta").modal();
            	obtenerCatalogoMotivoNoAltaImss();
          	}
        	}
      	},error: function (jqXHR, textStatus, errorThrown) {
        	alert(jqXHR.responseText);
	...
	function obtenerCatalogoMotivoNoAltaImss() {
	  $.ajax({
	    type: "POST",
	    ...
	    success: function (response) {
	      if (response.status == "success") {
	        ...
	        $("#selMotivoNoImss").empty().append('<option value="0">SELECCIONA..</option>');
	        $.each(datos, function (i) {
	        	$("#selMotivoNoImss").append('<option value="' +response.datos[i].idMotivo +'">' +response.datos[i].Descripcion +"</option>");
	        });
	      }
	    },
	    error: function (jqXHR, textStatus, errorThrown) {
	...
	function cargaerroresFirmaInternaImssAlta(mensaje) {
  	$("#errormodalFirmaAltaImss").fadeIn();
  	msjerrorbaja = "<div id='errormodalFirmaAltaImss1' class='alert alert-error'><strong>ALERTA:</strong> " + mensaje + " <a href='#' class='close' data-dismiss='alert'>&times;</a></div>";
  	$("#errormodalFirmaAltaImss").html(msjerrorbaja);
  	$(document).scrollTop(0);
  	$("#errormodalFirmaAltaImss").delay(4000).fadeOut("slow");
	}

	function cancelarFirmaAltaImss() {
	  $("#modalFirmaAltaImss").modal("hide");
	}

	function actualizarEstatusImssDefinitivo(){
		var numeroEmpleado=$("#txtNumEmpleadoDefinitivo").val();
		var comentario=$("#selMotivoNoImss").val();
		var contraseniaInsertadaCifrada = $("#constraseniaFirmaAltaImssHidden").val();
    var NumEmpModal = $("#NumEmpModalFirmaAltaImsshidden").val();
		var estatusImss=8;
		if (comentario == 0) {
  		var alertMsg1 = "<div id='msgAlert' class='alert alert-error'><strong>Error: </strong>Seleccione un motivo<a href='#' class='close' data-dismiss='alert'>&times;</a></div>";
        ...
        data: {"numeroEmpleado":numeroEmpleado,"estatusImss":estatusImss, comentario:comentario,contraseniaInsertadaCifrada: contraseniaInsertadaCifrada,NumEmpModal: NumEmpModal,},
        dataType: "json",
        success: function(response) {
          var mensaje=response.message;
          if (response.status=="success") {
