<div align="center">
  <h2>Actualización de Otros Pagos para Nomina</h2>
  <br>
  <select id="slPeriodoOPN" name="slPeriodoOPN" class="input-large ">
  ...
  <div class="card border-primary mb-3" style="max-width: 50rem;">
    ...
    <div class="card-body text-primary">
       <button class="btn-primary" onclick="getEmpleadosWithPagosN()">Descargar</button>
    </div>
  </div>

  <div class="card border-success mb-3" style="max-width: 50rem;">
    <div class="card-header"><h4>Actualizar Otros Pagos</h4></div>
	   <form enctype='multipart/form-data' id='pagosNomina' name='pagosNomina'>
    <div class="card-body text-primary">
			<label class="control-label label" for="deudorPagosN">Selecciona archivo: </label>
          <span class="btn btn-success btn-file" >Examinar
            <input type='file' class='btn-success' id='deudorPagosN' name='deudorPagosN'/> 
          </span>
    </div>            
    </form>
  ...
  <h4 id="hultimavezactualizaPagosN"></h4>
</div>
<script type="text/javascript">
  ...
  $('#deudorPagosN').change(function(){   
      var periodo=$('select[name="slPeriodoOPN"] option:selected').text();
      if(periodo=="PERIODO"){
        alert("Seleccione el periodo");
        $("#pagosNomina")[0].reset();
        return ;
      }        
      var file = $("#deudorPagosN")[0].files[0];      
        ...
        formData.append('tipo', 'pagosN'+periodo);
        formData.append('documento', 'deudorPagosN');
        ...
        var msj=verificarCierrePeriodo($("#slPeriodoOPN").val());
        if(msj!='correcto'){
...
function getEmpleadosWithPagosN()
{
        
//  alert(apellido);  
  var periodo=$("#slPeriodoOPN").val();
  if(periodo==""){
      alert("Seleccione el periodo");
      return 0;
  }
...
$("#slPeriodoOPN").change(function(){
var periodo=$('select[name="slPeriodoOPN"] option:selected').text();
consultarultimasubidaactualizapagosN(periodo);
});

function consultarultimasubidaactualizapagosN(periodo){
  $.ajax({
            type: "POST",
            ...
            dataType: "json",
             ...
             // console.log(response);
              if(response.datos.length!=0){
                ...
                $("#hultimavezactualizaPagosN").html("Última Carga de Archivo: "+ fecha);          
              }else{$("#hultimavezactualizaPagosN").html("Última Carga de Archivo: Sin Información");}
            },
            error: function(jqXHR, textStatus, errorThrown){
                  alert(jqXHR.responseText);
