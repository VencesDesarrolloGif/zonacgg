...
//$log = new KLogger ( "ajax_getCoberturaByRangoFecha.log" , KLogger::DEBUG );
...
if (!empty ($_POST))
{
	...
	$lista= $negocio -> getCoberturaRangoFecha($fecha1, $fecha2);

	for ($i = 0; $i < count($lista); $i++)
    {
        $puntoServicioAsistenciaId = $lista [$i]["puntoServicioAsistenciaId"];
        $puestoCubiertoId = $lista [$i]["puestoCubiertoId"];


        $lista [$i]["costoTurno"] = $negocio -> getCostoTurnoByPuntoIdAndPuestoId($puntoServicioAsistenciaId,$puestoCubiertoId );
