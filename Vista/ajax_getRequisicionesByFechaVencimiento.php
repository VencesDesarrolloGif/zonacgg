...
try{
	...
	$listaRequisiciones= $negocio -> ObtenerRequisicionesByFechaVencimientoXMes($FechaActual);
	$response["listaRequisiciones"]= $listaRequisiciones;
...
/*
	...
	try{
		...
		$listaRequisiciones= $negocio -> obtenerlistaRequisicionesSinBajaImss();
   
        for ($i = 0; $i < count($listaRequisiciones); $i++)
        {   
            $empleadoId = $listaRequisiciones[$i] ["numeroEmpleado"];
            $nombreEmpleado = $listaRequisiciones[$i] ["apellidoPaterno"];
            $fechaIngreso = $listaRequisiciones[$i] ["fechaIngresoEmpleado"];
            //$listaRequisiciones[$i] ["accion_confirmar_imss"] = "<a href='javascript:confirmar_imss(\"" . $empleadoId . "\",\"".$nombreEmpleado."\",\"".$fechaIngreso."\");'>
            //<img src='img/editarEmpleado.png' /></a>";
            //<img src=https://".$_SERVER["SERVER_NAME"].":".$_SERVER["SERVER_NAME"] ."/img/editarEmpleado.png' /></a>";
        }
        
		$response["data"]= $listaRequisiciones;
