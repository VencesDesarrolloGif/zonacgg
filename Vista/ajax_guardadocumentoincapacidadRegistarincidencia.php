...
verificarInicioSesion($negocio); 
 $log = new KLogger ( "ajax_UploadDocumentincapacidad.log" , KLogger::DEBUG );
...
$dictamen = getValueFromPost("dictamen");
...
if($CondicionFecha=="1"){
    if($st7=="" || $st7 == null || $st7 == "null" || $st7 == NULL || $st7 == "NULL" || ($tipoIncapacidad=="1" || $tipoIncapacidad=="3")){
        $st7="0";
    }
    if($st2=="" || $st2 == null || $st2 == "null" || $st2 == NULL || $st2 == "NULL" || ($tipoIncapacidad=="1" || $tipoIncapacidad=="3")){
        $st2="0";
    }
    if($dictamen=="" || $dictamen == null || $dictamen == "null" || $dictamen == NULL || $dictamen == "NULL" || ($tipoIncapacidad=="1" || $tipoIncapacidad=="3")){
        $dictamen="0";
    }
    $empleado ["st7"] = $st7;//st7
    $empleado ["st2"]  = $st2;//st2
    $empleado ["dictamen"]  = $dictamen;//st2
    if($opcion==0){
        $response["status"] = "success1";
        $response ["message"] ="";
        $resultado=$negocio -> insertandupdatefolioincapacidad($folioIncapacidad,$asistenciaFecha,$fechafinalincidencia,$empleado,$tipoIncapacidad,$diasIncapacidad,$opcion);
        ...
        //update para agregar el folio en asistencia e insert en la nueva tabla de folio 
            $negocio -> insertandupdatefolioincapacidad($folioIncapacidad,$asistenciaFecha,$fechafinalincidencia,$empleado,$tipoIncapacidad,$diasIncapacidad,$opcion);
        }
    }else if($opcion==2){
        $response["status"] ='success2';
        $response["message"]='';
        $conteoincapacidades2= $negocio -> insertandupdatefolioincapacidad($folioIncapacidad,$asistenciaFecha,$fechafinalincidencia,$empleado,$tipoIncapacidad,$diasIncapacidad,$opcion);
        if(count($conteoincapacidades2)!=0){
           ...
           $response["Dictamen"]= $conteoincapacidades2[0]["Dictamen"];
        ...
        $permitidosjpg= "application/jpg";
        $correcto=true;
        $num=count($_FILES["docuDic"]['type']);
        for($a=0;$a<$num;$a++){
            $valor=$_FILES["docuDic"]['type'][$a];
            if($valor!=$permitidospdf && $valor!=$permitidosjpeg && $valor!=$permitidospng && $valor!=$permitidosjpg){
                $correcto=false;
                break;
            }
        ...
        if(!$correcto){
            $response["status"] = "error";
            $response ["message"] = "Tipos de arhivos DICTAMEN son correcto o limite excedido"; 
        }else{
            $extencionArchivo= explode("/", $_FILES["docuDic"]['type'][0]);    
            $extencionArchivooriginal=$extencionArchivo[1];
            foreach($_FILES["docuDic"]['tmp_name'] as $key => $tmp_name)
            {
                if($_FILES["docuDic"]["name"][$key]) {
                    $filename = $_FILES["docuDic"]["name"][$key]; //Obtenemos el nombre original del archivo
                    $source = $_FILES["docuDic"]["tmp_name"][$key]; //Obtenemos un nombre temporal del archivo          
                    $directorio = 'uploads/DocumentosIncapacidad/'.$empleado ["entidadId"]."-". $empleado ["consecutivoId"]."-".$empleado ["tipoId"]."/"; //Declaramos un  variable con la ruta donde guardaremos los archivos
                    if(!file_exists($directorio)){
                        mkdir($directorio, 0777) or die("No se puede crear el directorio de extracci&oacute;n");    
                    }
                    $dir=opendir($directorio); //Abrimos el directorio de destino
                    $target_path = $directorio.'/'.$folioIncapacidad."_DICTAMEN-".$tipoIncapacidad."_".$asistenciaFecha."_".$fechafinalincidencia.".".$extencionArchivooriginal; //Indicamos la ruta de destino, así como el nombre del archivo
                    if(!move_uploaded_file($source, $target_path)) { 
                        $response["status"] = "error";
                        $response ["message"] = "Error al subir archivos dictamen";
                    }
                    closedir($dir); //Cerramos el directorio de destino
                }
            }
