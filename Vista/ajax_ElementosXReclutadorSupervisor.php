...
//$log = new KLogger ( "ajaxObtenerReclutadores.log" , KLogger::DEBUG );
	...
	try{
		$listaEmpleados= $negocio -> ObtenerReclutadoresByLineaNegocioAndMonth($lineaNegocio, $month,$puesto);
		//$log->LogInfo("Valor de la variable \$response punto: " . var_export ($listaEmpleados1, true));
        $elementosReclutados="";
        ...
        {   //$numeroEmpleadoNOM = $listaEmpleados[$i] ["numeroEmpleadoNombre"];
           ...
           //$fechaIngresoEmpleado = $listaEmpleados[$i] ["fechaIngresoEmpleado"];
            $elementosReclutados= $negocio -> ObtenerElementosByReclutador($numeroEmpleado, $month);
