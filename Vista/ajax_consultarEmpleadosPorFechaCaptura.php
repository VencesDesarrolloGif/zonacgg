...
//$log = new KLogger ( "ajaxConsultaEmpleadosFechaCaptura.log" , KLogger::DEBUG );
...
try{
	...
	$listaEmpleados= $negocio -> obtenerListaEmpleadosPorFechaCaptura($dia, $mes, $anio);
	for ($i=0; $i < count($listaEmpleados) ; $i++) { 
		$empleadoEstatusId = $listaEmpleados[$i]["empleadoEstatusId"];
		if($empleadoEstatusId=="2"){
			$listaEmpleados[$i]["UsuarioMovimineto"] = $listaEmpleados[$i]["nombreUsuarioCaptura1"];
		}else{
			$listaEmpleados[$i]["UsuarioMovimineto"] = $listaEmpleados[$i]["nombreUsuarioCaptura"];
		}
	}
