        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.status == "success") {
                    ...
                    var tabla = "<table id='tabla' class='table table-bordered'><thead><th>No</th><th>Para ingresos de</th><th>Hasta ingresos de</th><th>Cantidad de subsidio para el empleo mensual</th></thead><tbody>";
                    $(document).scrollTop(0);
                    $.each(datos, function(i) {
                        tabla += "<tr><td > " + (i + 1) + " </td>";
                        tabla += "<td ><input id='inpparaingresos" + i + "' type='text' readonly='true' value='" + datos[i].ParaIngMen + "'></td>";
                        tabla += "<td><input id='inphastaingresos" + i + "' type='text' readonly='true' value='" + datos[i].HasIngMen + "'></td>";
                        tabla += "<td><input  id='inpcantidadsubsidios" + i + "' type='text' readonly='true' value='" + datos[i].cantSubsidioMensual + "'></td>";
                    });
                    $("#datos").append(tabla);
                } else {
                    var mensaje = response.message;
...
function guardar() {
    ...
    if (inpaccion === "Agregar") {
        ...
        /*else if (sobreexcedente == "" || !/^([0-9]+\.?[0-9]{0,2})*$/.test(sobreexcedente)) {
            ...
            $.ajax({
                type: "POST",
                url: 'insertyupdateatblsubsidioMensual.php',
                data: {
