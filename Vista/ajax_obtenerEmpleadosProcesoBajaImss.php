...
//$log = new KLogger ( "ajaxObtenerEnProcesoBajaImss.log" , KLogger::DEBUG );
//$estatusEmpleado=getValueFromPost("estatusEmpleado");

try {

    $listaEmpleados = $negocio->obtenerListaEmpleadosEnProcesoBaja();
    ...
    for ($i = 0; $i < count($listaEmpleados); $i++) {
        ...
        $fechaImss        = $listaEmpleados[$i]["fechaBajaImss"];
        $registroPatronal = $listaEmpleados[$i]["registroPatronal"];
        $folioTxt         = $listaEmpleados[$i]["folioTxtBaja"];
        $rechazarbajaimss = $listaEmpleados[$i]["accion_confirmar_imss_baja"] = "<a href='javascript:confirmar_imss_rechazo_baja(\"" . $empleadoId . "\",\"" . $nombreEmpleado . "\",\"" . $fechaImss . "\");'><img src='img/cancel.png' /></a>";
