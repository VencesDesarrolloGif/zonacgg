...
//$log = new KLogger ( "ajax_obtenerTotalTarjetasDisponiblesPorEntidadParaStock.log" , KLogger::DEBUG );
    $sql = "SELECT * from TarjetaDespensa
            ...
            left join sucursalesinternas on (TarjetaDespensa.idEntidadAsignada=sucursalesinternas.idSucursalI)
            left join entidadesfederativas on (sucursalesinternas.idEntidadPerteneciente=entidadesfederativas.idEntidadFederativa)
            ...
            if($IdEntidadARevisar!="General1"){
                $sql.=" where idEntidadAsignada='$IdEntidadARevisar'";
            }
    ...
    for($i = 0; $i < count($datos); $i++){
        ...
        if($IdAsignacionEmpleado =="1" || $IdAsignacionEmpleado =="0"){
