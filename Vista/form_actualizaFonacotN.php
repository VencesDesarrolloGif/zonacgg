<div align="center">
  <h2>Actualización de Fonacot para Nominas</h2>
  <br>
  <select id="slPeriodoFN" name="slPeriodoFN" class="input-large ">
  ...
  <div class="card border-primary mb-3" style="max-width: 50rem;">
    ...
    <div class="card-body text-primary">
       <button class="btn-primary" onclick="getEmpleadosWithFonacotN()">Descargar</button>
    </div>
  </div>

  <div class="card border-success mb-3" style="max-width: 50rem;">
    <div class="card-header"><h4>Actualizar Fonacot</h4></div>
	   <form enctype='multipart/form-data' id='fonacotNomina' name='fonacotNomina'>
    <div class="card-body text-primary">
			<label class="control-label label" for="deudorFonacotN">Selecciona archivo: </label>
          <span class="btn btn-success btn-file" >Examinar
            <input type='file' class='btn-success' id='deudorFonacotN' name='deudorFonacotN'/> 
          </span>
    </div>            
    </form>
  ...
  <h4 id="hultimavezfonacotnomina"></h4>
</div>
<script type="text/javascript">
  ...
  $('#deudorFonacotN').change(function(){   
      var periodo=$('select[name="slPeriodoFN"] option:selected').text();
      if(periodo=="PERIODO"){
        alert("Seleccione el periodo");
        $("#fonacotNomina")[0].reset();
        return ;
      }        
      var file = $("#deudorFonacotN")[0].files[0];      
        ...
        var formData = new FormData($("#fonacotNomina")[0]);
        formData.append('tipo', 'fonacotN'+periodo);
        formData.append('documento', 'deudorFonacotN');
        ...
        var msj=verificarCierrePeriodo($("#slPeriodoFN").val());
        if(msj!='correcto'){
  ...
  function actualizaFonacotNomina(){
    var periodo=$('select[name="slPeriodoFN"] option:selected').text();
    //alert("actualizando");
    waitingDialog.show();
  		$.ajax({
...
function getEmpleadosWithFonacotN()
{
        
//  alert(apellido);  
  var periodo=$("#slPeriodoFN").val();
  if(periodo==""){
      alert("Seleccione el periodo");
      return 0;
  }
...
$("#slPeriodoFN").change(function(){
var periodo=$('select[name="slPeriodoFN"] option:selected').text();
consultarultimasubidadeduccionfonacotnomina(periodo);
});

function consultarultimasubidadeduccionfonacotnomina(periodo){
  $.ajax({
            type: "POST",
            ...
            dataType: "json",
             ...
             // console.log(response);
              if(response.datos.length!=0){
                ...
                $("#hultimavezfonacotnomina").html("Última Carga de Archivo: "+ fecha);          
              }else{$("#hultimavezfonacotnomina").html("Última Carga de Archivo: Sin Información");}
            },
            error: function(jqXHR, textStatus, errorThrown){
                  alert(jqXHR.responseText);
