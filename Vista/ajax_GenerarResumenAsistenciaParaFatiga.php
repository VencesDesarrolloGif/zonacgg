...
try{
    ...
    $turnosDeNoche = 0;
    $caso="1";
    $lista = $negocio -> getDetalleRequisicionesByPuntoServicioIdParaFatiga($puntoServicioId,$IdPlantillaServ,$caso);
    ...
    $result = array ();
    $result [] = "";
    $listaTurnosCubiertos = $negocio -> getTurnosCubiertosByPeriodoFechasAndPuntoServicioParaFatiga ($fechaInicial, $fechaFinal, $puntoServicioId ,$IdPlantillaServ); 
    $listaTurnosCubiertosIE = $negocio -> obtenerTurnoCubiertosIncidenciasEspecialesXDiaParaFatiga ($fechaInicial, $fechaFinal, $puntoServicioId ,$IdPlantillaServ);// esta 
    for ($i =$fechaInicial; $i <=$fechaFinal; $i = date("Y-m-d", strtotime($i ."+ 1 days")))
    {      
        $DatosPlantillasPorPunto = $negocio -> GetDatosPlantillasPorPuntoParaFatiga($puntoServicioId ,$IdPlantillaServ);
        ...
        if($LineaNegocioPunto=="1"){
            ...
            for($j=0; $j<$LargoDatosplantilla;$j++){
                ...
                $DiasSolicitud= $negocio -> getDiasSolicitadosParaFatiga($PlantillaId,$turnoDiaC,$turnoNocheC);
            ...
            $listaTurnosCubiertosSeparados = $negocio -> getTurnosCubiertoSeparadosXPuntosParaFatiga($i, $puntoServicioId,$IdPlantillaServ);
        ...
        if($turnosPorDia==0 || $turnosPorDia=="" || $turnosPorDia==null){
            $RevisionTipoTurno = $negocio -> getTTipoTurnoCurbiertoParaFatiga($i, $puntoServicioId,$IdPlantillaServ);
