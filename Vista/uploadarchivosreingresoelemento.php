...
//$log = new KLogger ( "ajaxuploadarchivosreingresos.log" , KLogger::DEBUG );
//$log->LogInfo("Valor de la variable \$filename : " . var_export ($_FILES, true));
//$log->LogInfo("Valor de la variable \$filename : " . var_export ($_POST, true));
$docdigitalizadoo3=$_FILES["docdigitalizado3"]['type'][0];
$iteracion=4;
$response = array ();
...
$numeroEmpleado=$_POST["txtNumeroEmpleadoModalR"];
...
//$log->LogInfo("Valor de la variable \$filename : " . var_export ($iteracion, true));
//$listaDeDOcumentos=$negocio -> negocio_listarDocumentosDigitalizados($entidadEmpleado,$consecutivoEmpleado,$categoriaEmpleado);
    ...
    {
        $valor=$_FILES["docdigitalizado".$a]['type'][0];
    if(!$correcto)
    {
    ...
    {
        foreach($_FILES["docdigitalizado".$a]['tmp_name'] as $key => $tmp_name)
        {
            if($_FILES["docdigitalizado".$a]["name"][$key])
            {
                $filename = $_FILES["docdigitalizado".$a]["name"][$key]; //Obtenemos el nombre original del archivo
                $source = $_FILES["docdigitalizado".$a]["tmp_name"][$key]; //Obtenemos un nombre temporal del archivo          
