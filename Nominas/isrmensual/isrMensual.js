            ...
            success: function(response) {
                ...
                if (response.status == "success") {
                    ...
                    $.each(datos, function(i) {
                        tabla += "<tr><td > " + (i + 1) + " </td>";
                        tabla += "<td ><input id='inplimiteinferior" + i + "' type='text' readonly='true' value='" + datos[i].limiteInferior + "'></td>";
                        tabla += "<td><input id='inplimitesuperior" + i + "' type='text' readonly='true' value='" + datos[i].limiteSuperior + "'></td>";
                        tabla += "<td><input  id='inpcuotafija" + i + "' type='text' readonly='true' value='" + datos[i].cuotaFija + "'></td>";
                        tabla += "<td><input id='inpsobreexcedente" + i + "' type='text' readonly='true' value='" + datos[i].sobreExcedenteLimInferior + "' ></td></tr>"
                    });
                    $("#datos").append(tabla);
                } else {
                    var mensaje = response.message;
...
function guardar() {
    ...
    if (inpaccion === "Agregar") {
        ...
        if (limiteinferior == "" || !/^([0-9]+\.?[0-9]{0,2})*$/.test(limiteinferior)) {
            ...
            $.ajax({
                type: "POST",
                url: 'insertyupdateatblisrMensual.php',
                data: {
