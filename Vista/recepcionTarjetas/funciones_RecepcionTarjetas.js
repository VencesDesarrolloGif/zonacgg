...
$("#selectTipoRecepcion").change(function(){
  ...
  if(tipoRecepcion=='1'){//de alguna matriz gif A Una Entidad
     waitingDialog.show();
    ...
    $("#divRecepcionTarjetasInternaParaMatrizR").hide();
     ...
     $("#SucursalParaRecibirTarjetas").hide();
     $("#divRecepcionTarjetasInternaParaMatrizR").hide();
  }else if (tipoRecepcion=='3'){//Recpecion A Matriz Mandada Por Una Entidad
     $("#divRecepcionTarjetasInternas").hide();
     $("#SucursalParaRecibirTarjetas").hide();
     $("#divTablaRecepcionExterna").hide();
     waitingDialog.show();
...
function ActualizarRecepcionTarjetasParaMatrizR(){
    ...
    var tarjetaARecibirEnMatriz = [];
    for (var i = 0; i < tarjetasSeleccionadasMatrizR.length; i++)
    {
        if (tarjetasSeleccionadasMatrizR[i].checked == true)
        {
            tarjetaARecibirEnMatriz.push (tarjetasSeleccionadasMatrizR[i].value);
        }
    }
    if (tarjetaARecibirEnMatriz.length > 0)
    {
        $.ajax({
            type: "POST",
            url: "recepcionTarjetas/ajax_ActualizarRecepcionDeTarjetasMatrizR.php",
            data:{"tarjetaARecibirEnMatriz":tarjetaARecibirEnMatriz,"NumEmpModalFirmaParaEntidad":NumEmpModalFirmaParaEntidad,"constraseniaFirmaParaEntidad":constraseniaFirmaParaEntidad},
            dataType: "json",
            async: false,
            success: function(response) {
            },
...
$("#SelectEntidadDeRecepcion").change(function(){
    ...
    $.ajax({            
      type:"POST",
      url: "recepcionTarjetas/ajax_ObtenerSucursalesPorEntidadParaRecepcion.php",
      data:{'EntidadABuscar': EntidadABuscar},
      dataType: "json",
      success: function(response) {
        var datos = response.datos;
        $('#SelectSucursalDeRecepcion').empty().append('<option value="0" selected="selected">Sucursal</option>');
        $.each(datos, function(i) {
            $('#SelectSucursalDeRecepcion').append('<option value="' + response.datos[i].idSucursalI+'">' + response.datos[i].nombreSucursal + '</option>');
        });
        waitingDialog.hide();     
      },
      error: function(jqXHR, textStatus, errorThrown) {
    ...
    $("#SucursalParaRecibirTarjetas").show();
});

$("#SelectSucursalDeRecepcion").change(function(){
    var EntidadABuscar1 = $("#SelectSucursalDeRecepcion").val();
    obtenerTotalTarjetasDisponiblesEnCadaEntidadParaRecibir(EntidadABuscar1);
});
...
function ActualizarRecepcionTarjetasParaEntidad(){
    ...
    var tarjetaAEnviarARecibirEnEntidad = [];

    for (var i = 0; i < tarjetasSeleccionadas.length; i++)
    {
        if (tarjetasSeleccionadas[i].checked == true)
        {
            tarjetaAEnviarARecibirEnEntidad.push (tarjetasSeleccionadas[i].value);
        }
    }
    if (tarjetaAEnviarARecibirEnEntidad.length > 0)
    {
        $.ajax({
            type: "POST",
            url: "recepcionTarjetas/ajax_ActualizarRecepcionDeTarjetasPorEntidad.php",
            data:{"tarjetaAEnviarARecibirEnEntidad":tarjetaAEnviarARecibirEnEntidad,"NumEmpModalFirmaParaEntidad":NumEmpModalFirmaParaEntidad,"constraseniaFirmaParaEntidad":constraseniaFirmaParaEntidad},
            dataType: "json",
            async: false,
            success: function(response) {
            },
