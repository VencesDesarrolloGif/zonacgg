$(traedatos()); 
    function traedatos() {
        $.ajax({
            type: "POST",
            url: "ajax_cattipoincidencia.php",
            dataType: "json",
            success: function(response) {
                ...
                if (response.status == "success") {
                    ...
                    $.each(datos, function(i) {
                        tabla += "<tr><td > <input  id='inpid" + i + "' type='text' readonly='true' value='" + datos[i].numTipoIncapacidad + "'> </td>";
...
function guardar() {
    ...
    if (inpaccion === "Agregar") {
        ...
        if (id == "") {
            $(document).scrollTop(0);
            var Msgerrorfechatblsalarios = "<div id='errorMsgtblsalarios' class='alert alert-error'><strong>Verifique Id: </strong>" + " <a href='#' class='close' data-dismiss='alert'>&times;</a></div>";
            ...
            $.ajax({
                type: "POST",
                url: 'insertyupdatecattipoincidencia.php',
                data: {
        ...
        for (var i = 0; i < b - 1; i++) {
            ...
            if (id[i] == "") {
                $(document).scrollTop(0);
                var Msgerrorfechatblsalarios = "<div id='errorMsgtblsalarios' class='alert alert-error'><strong>Verifique Id en la fila:" + (i + 1) + "</strong>" + " <a href='#' class='close' data-dismiss='alert'>&times;</a></div>";
                $("#errorMsgtblsalarios").html(Msgerrorfechatblsalarios);
            ...
            if (descripcion[i] == "") {
                $(document).scrollTop(0);
                var Msgerrorfechatblsalarios = "<div id='errorMsgtblsalarios' class='alert alert-error'><strong>Verifique Descripción en la fila:" + (i + 1) + "</strong>" + " <a href='#' class='close' data-dismiss='alert'>&times;</a></div>";
                $("#errorMsgtblsalarios").html(Msgerrorfechatblsalarios);
                return 0;
            }
        }
